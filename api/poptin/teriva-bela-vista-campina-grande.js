...
const CLIENT_LISTING_ID = "Campina Grande";
...
export default async function handler(req, res) {
  ...
  await postJson(AUDIT_URL, {
    secret: AUDIT_SECRET,
    receiptId,
    empreendimento: "teriva-bela-vista-campina-grande",
    nome: name,
    telefone: phone,
    email,
    status: "RECEIVED",
  ...
  const lastroPayload = {
    ...
    origin: ORIGIN,
    formText: `Lead via Poptin | Teriva Bela Vista Campina Grande | Página: ${url || "(sem url)"} | Mensagem: ${message || "(sem mensagem)"}`,
  };
